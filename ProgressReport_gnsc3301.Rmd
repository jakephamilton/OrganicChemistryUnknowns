...
Summary of project:
  We took the compounds from the list of possible Organic Chemistry Lab Unknowns, and compiled them in an Excel spreadsheet, including for each compound the melting point, boiling point, physical state at room temperature, and any notable functional groups that were included in each compound. This spreadsheet was then saved as a .csv file so the values could be queried from the spreadsheet in an SQL database. Finally, a Python code was written to create a search engine that would query chemical compounds that could be selected based on specified physical state, melting point, boiling point, and functional groups.
...
Jake: creating the group GitHub repository, entering the information for my part of the compounds in the Excel spreadsheet, writing about 40% of the python code (with assistance from Dr. Malmberg), writing some of the descriptions of our project in the RMarkdown document.
